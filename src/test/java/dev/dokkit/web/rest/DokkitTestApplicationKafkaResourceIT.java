...
import dev.dokkit.IntegrationTest;
import dev.dokkit.config.EmbeddedKafka;
import dev.dokkit.config.KafkaSseConsumer;
import dev.dokkit.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class DokkitTestApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/dokkit-test-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dokkit-test-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/dokkit-test-application-kafka/unregister"));
                return;
            }
        }
